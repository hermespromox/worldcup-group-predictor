...
import pandas as pd
from .elo import compute_elo_ratings, opponent_adjusted_form
...
def compute_team_features(results: pd.DataFrame, teams: list[str], cutoff: str | pd.Timestamp, cfg: dict) -> pd.DataFrame:
    elo_ratings = compute_elo_ratings(
        ...
        teams,
        cutoff,
        variant=str(cfg.get("elo_variant", "advanced")),
        home_advantage=float(cfg.get("home_advantage", 65.0)),
        decay_half_life_days=cfg.get("elo_decay_half_life_days", 3650.0),
    )
    adj_form = opponent_adjusted_form(results, teams, cutoff, elo_ratings, n=int(cfg.get("opponent_form_window", 10)))
    ...
    for team in teams:
        ...
        if used == 0:
            rows.append({"team": team, "elo": elo_ratings.get(team, 1500.0), "opponent_adjusted_form": adj_form.get(team, 0.0), "matches_used":0, "neutral_matches_used":0, "sparse_data_flag": True,
        ...
        rows.append({"team":team,"elo": elo_ratings.get(team, 1500.0),"opponent_adjusted_form": adj_form.get(team, 0.0),"matches_used":used,"neutral_matches_used":len(neutral),"sparse_data_flag":sparse,
